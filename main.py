...
app.config['SQLALCHEMY_DATABASE_URI']='mysql+pymysql://build-a-blog:1111@localhost:8889/build-a-blog'
...
def index():
    ...
    if request.method == 'POST':
        ...
        if blogpost=='':
            blogpost_error='<-----Dude or Dudess, this cannot be blank.  Try again!'
    ...
    return render_template('index.html', ptitle='Launchcode Tips and Tricks', title=title, blogpost=blogpost, title_error=title_error, blogpost_error=blogpost_error)

@app.route('/allblogs', methods=['POST', 'GET'])
def blog():

    return render_template('allblogs.html',title="Coding Tips and Tricks Blog - All Entries", allblogs=get_allblogs())
